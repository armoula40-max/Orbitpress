...
// Adapt the supplied long-form example to every niche available in Studio.
// The section vocabulary also matches the anchors used by inline image placement.
const NICHE_BLUEPRINTS = {
  food: 'Introduction; Ingredients and Materials; Preparation and Timing; Step-by-Step Instructions; Serving, Storage, and Make-Ahead Tips; Common Mistakes to Avoid; FAQs; Conclusion',
  crochet: 'Introduction; Materials and Supplies; Sizing, Gauge, and Preparation; Step-by-Step Instructions; Customization and Alternative Methods; Common Mistakes to Avoid; Care and Storage; FAQs; Conclusion',
  pets: 'Introduction; Supplies and Safety; Preparation and Routine; Step-by-Step Instructions; Training or Care Variations; Common Mistakes to Avoid; Maintenance and When to Seek Professional Help; FAQs; Conclusion',
  nails: 'Introduction; Tools and Products; Preparation and Timing; Step-by-Step Instructions; Design Variations and Removal; Common Mistakes to Avoid; Maintenance and Aftercare; FAQs; Conclusion',
  furniture: 'Introduction; Materials, Measurements, and Tools; Planning and Preparation; Step-by-Step Instructions; Layout and Alternative Options; Common Mistakes to Avoid; Maintenance and Buying Considerations; FAQs; Conclusion',
  'home-decor': 'Introduction; Materials and Supplies; Planning and Preparation; Step-by-Step Instructions; Style Variations and Small-Space Options; Common Mistakes to Avoid; Maintenance and Seasonal Updates; FAQs; Conclusion',
  diy: 'Introduction; Materials, Tools, and Safety; Planning and Preparation; Step-by-Step Instructions; Alternative Methods and Customization; Common Mistakes to Avoid; Finishing, Maintenance, and Storage; FAQs; Conclusion',
  beauty: 'Introduction; Products, Tools, and Safety; Preparation and Timing; Step-by-Step Instructions; Variations for Different Needs; Common Mistakes to Avoid; Maintenance and Aftercare; FAQs; Conclusion',
  gardening: 'Introduction; Materials and Supplies; Timing and Growing Schedule; Step-by-Step Instructions; Container, Indoor, and Organic Alternatives; Common Mistakes to Avoid; Storage and Ongoing Maintenance; FAQs; Conclusion',
};
...
function articleResponseFormat() {
  const schema = {
    ...
    properties: {
      ...
      pinterest: { type: 'object', properties: { title: { type: 'string' }, description: { type: 'string' }, altText: { type: 'string' }, hashtags: { type: 'array', items: { type: 'string' }, maxItems: 5 }, topics: { type: 'array', items: { type: 'string' }, maxItems: 8 } }, required: ['title', 'description', 'altText', 'hashtags', 'topics'], additionalProperties: false },
    },
...
async function generate(request) {
  ...
  const maxTokens = requestedRecipeCount > 0
    ? Math.min(30000, Math.max(18000, requestedRecipeCount * 2200))
    : 12000;
  // Repairs need a little more room than the first pass because they receive
  // the previous JSON as context and must return the complete normalized
  // document again. This is especially important when a long keyphrase makes
  // the user prompt substantially larger.
  const repairMaxTokens = requestedRecipeCount > 0
    ? Math.min(30000, Math.max(20000, requestedRecipeCount * 2400))
    : maxTokens;
  ...
  let completeness = requestedRecipeCount > 0
    ...
    : { valid: true, reason: '' };
  let repairPasses = 0;
  while (!completeness.valid && repairPasses < 3) {
    ...
    const repaired = await requestArticleCompletion({ settings, messages: repairMessages, temperature: 0.35, maxTokens: repairMaxTokens });
...
function buildPrompt({ keyword, niche, requestedType, category, keywords, titleList, requestedRecipeCount, customizedTextPrompt, internalCandidates }) {
  ...
  const blueprint = NICHE_BLUEPRINTS[niche] || NICHE_BLUEPRINTS.diy;
  const relatedKeyword = keywords.find((item) => SeoAnalyzer.norm(item) !== SeoAnalyzer.norm(keyword)) || '';
  return `
    ...
    Recommended section blueprint for this niche: ${blueprint}
    ...
    - seoTitle CTR: if the body genuinely delivers a numbered set (N tips, N steps, N recipes, N ways, N mistakes), put that REAL number in WESTERN digits (1-9, e.g. "7 نصائح" not "٧ نصائح" — analyzers detect ASCII digits) at or near the start of seoTitle — never invent a number for an article that is not a list, and never add one to a general informational article. Use ONE truthful Arabic power word only when the content really supports it (سهلة، سريعة، مجربة، لذيذة، عملية، شاملة، فعالة، مثالية، ناجحة، خطوة بخطوة) — no hype the body does not deliver. Note: automated title-sentiment analysis only understands English; for Arabic a truthful power word like "سهلة" or "مجربة" still satisfies real readers.
    ...
    - Use the focus keyphrase exactly (same word order) at least once; surrounding synonyms elsewhere. When a related keyword is available (${relatedKeyword || 'none supplied'}), include both the focus and related keyword naturally in the introduction, the step-by-step/action section, and the conclusion. Do not force repetition, and never add a keyword to a title merely to satisfy a count.
    - Follow the recommended blueprint when it fits the search intent. Keep headings specific to the selected niche: gardening may discuss a growing schedule and harvest, food may discuss ingredients and serving, crafts may discuss materials and finishing, pets may discuss safety and maintenance, and beauty/nails may discuss products and aftercare. Do not copy gardening or recipe sections into unrelated niches.
    - Image placement: write useful text under the relevant Materials/Ingredients, Preparation, Instructions, Tips, Storage/Maintenance, or Conclusion headings. Do not insert image URLs, <img> tags, n8n expressions, WordPress media expressions, or fake href values; the publishing pipeline adds the selected featured and inline images and their alt text after generation.
    ...
    # STRICT PINTEREST SEO FIELD
    - pinterest.title: a clear, compelling title of 40-100 characters that contains the exact focus keyphrase naturally near the beginning. Do not use vague hooks such as "you need this" without the topic.
    - pinterest.description: 250-500 characters of useful, natural search copy. Include the exact focus keyphrase once, 2-4 relevant secondary/long-tail phrases, a concrete benefit, and one honest CTA such as "Read the full guide". End with 2-5 relevant hashtags only; never repeat a hashtag or stuff keywords. Do not invent search volume, popularity, Pinterest Topics, claims, or URLs.
    - pinterest.hashtags: return 2-5 unique, topic-specific hashtags without spaces or punctuation beyond the leading #. They must be derived from the focus/secondary keywords, not generic tags such as #viral or #fyp.
    - pinterest.topics: return up to 8 concise Pinterest topic/category phrases for later manual or supported topic selection. Topics are suggestions only; never claim that they were applied automatically.
    - pinterest.altText: one accessible descriptive sentence of the actual image concept, including the focus keyphrase where natural (max 180 chars). Do not stuff keywords or mention Pinterest SEO.
    - Pinterest image guidance: prefer a readable vertical composition with the key topic visible, strong contrast, and safe margins for mobile. OrbitPress accepts flexible image sizes and must not crop a tall source just to force 2:3.

    # HARD CONSTRAINTS
    - Return valid JSON ONLY with this exact structure:
    {"title":"","metaDescription":"","focusKeyphrase":"","secondaryKeywords":[""],"seoTitle":"","seoDescription":"","paaQuestions":[""],"externalReferences":[{"anchor":"","topic":""}],"slug":"","contentType":"recipe|article","categoryName":"","outline":[{"heading":"","keyPoints":[""]}],"htmlContent":"","internalLinks":[{"anchor":"","reason":""}],"recipe":{"isRecipe":false,"description":"","prepTime":"","cookTime":"","totalTime":"","recipeYield":"","cuisine":"","ingredients":[],"instructions":[{"name":"","text":""}],"notes":[]},"recipes":[{"title":"","isRecipe":true,"description":"","prepTime":"","cookTime":"","totalTime":"","recipeYield":"","cuisine":"","ingredients":[""],"instructions":[{"name":"","text":""}],"notes":[""]}],"pinterest":{"title":"","description":"","altText":"","hashtags":[""],"topics":[""]}}

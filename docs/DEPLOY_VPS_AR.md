...
| Pinterest: النشر بالجلسة يفشل بـ 401 `"code":2"` أو زر «إعادة فحص الاتصال» يُظهر «غير متصل» رغم أن الماسح يعمل | الفحص القديم اعتمد على كوكي `_auth=1` الذي لا تستلمه بعض الحسابات الصالحة، فخفّض حالة الجلسة خطأً. أصبح التحقق وظيفياً عبر `UserResource/get` (بيانات المستخدم) مع إعادة محاولة للشبكة. حدّث النسخة ثم اضغط «إعادة فحص الاتصال» لتعود «متصل» دون إعادة دخول؛ إن بقيت غير متصلة أعد الدخول أو استورد الكوكيز. كما حُدّث مسار الرفع إلى تدفق S3 الحالي بعد إلغاء Pinterest لنقطة `/upload-image/` القديمة |
